...
use pgsavis\smssabanovin\Exceptions\ApiException;
use pgsavis\smssabanovin\Exceptions\HttpException;
use pgsavis\smssabanovin\Exceptions\RuntimeException;
...
class smssabanovin
{
    ...
    public function __construct()
    {
        ...
        $this->apiKey = config('smssabanovin.apiKey');
    ...
    public function Send( $to, $text, $at = null)
    {
        if (is_array($to)) {
            $to = implode(",", $to);
        }
        ...
        $params = array(
            "to" => $to,
            "gateway" => config('smssabanovin.gateway'),
            "text" => $text,
        );
        return $this->execute($path, $params);
    }

    public function SendArray( $to, $text, $at = null)
    {
        if (!is_array($to)) {
            $to = (array) $to;
        }
    ...
    public function Receive($is_read = 0)
    {
